...
| Voice Cloning | `LA_STUDIO_VOICE_CLONE_GPU.ipynb`; OmniVoice profile/generation jobs | No supported Gateway adapter in this codebase | Colab notebook, URL/token, consent and profile fields | Needs consented reference-audio live test. |
| Voice Design | `LA_STUDIO_VOICE_DESIGN_GPU.ipynb`; `qwen3-tts-1.7b-voicedesign` | No supported Gateway adapter in this codebase | Colab notebook, URL/token, model parameters | Needs live generation test. |
